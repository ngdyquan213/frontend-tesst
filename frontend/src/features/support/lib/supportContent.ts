...
export const supportHelpTopics: HelpTopic[] = [
  {
    id: 'bookings',
...
export function getSupportTopicLabel(topicId?: string) {
  return supportHelpTopics.find((topic) => topic.id === topicId)?.title ?? 'General support'
}
